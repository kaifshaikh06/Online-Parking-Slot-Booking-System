...
const getBookingDay = (booking) => new Date(booking.bookingDate).toISOString().slice(0, 10);
...
const isBookingActive = (booking, now = new Date()) => (
  booking.status === 'Booked' && getBookingStartDateTime(booking) <= now && getBookingEndDateTime(booking) > now
);
...
const syncSlotAvailability = async (parkingSlotId, now = new Date()) => {
  const bookings = await Booking.find({ parkingSlot: parkingSlotId, status: 'Booked' }).select('bookingDate startTime endTime status');
...
const releaseExpiredBookings = async () => {
  const now = new Date();
  const bookings = await Booking.find({ status: 'Booked' }).select('parkingSlot bookingDate startTime endTime status');
  const scheduledSlotIds = [...new Set(
    bookings.map((booking) => booking.parkingSlot.toString())
  )];

  await Promise.all(scheduledSlotIds.map((parkingSlotId) => syncSlotAvailability(parkingSlotId, now)));
...
module.exports = { getBookingDay, getBookingStartDateTime, getBookingEndDateTime, isBookingActive, isBookingReserved, syncSlotAvailability, releaseExpiredBookings, startBookingExpiryScheduler };
