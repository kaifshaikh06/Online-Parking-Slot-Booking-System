...
const seedSlots = async () => {
  ...
  const result = await ParkingSlot.bulkWrite(
    defaultSlots.map((slot) => ({
      updateOne: {
        ...
        update: {
          $set: { location: "Level A", vehicleType: "Sedan" },
          $setOnInsert: slot,
        },
        upsert: true,
      },
  ...
  await ParkingSlot.updateMany(
    {
      vehicleType: {
        $nin: ["Sedan", "SUV", "Coupe", "Muscle", "Hatchback", "Convertible"],
      },
    },
    { $set: { vehicleType: "Sedan" } },
  );
  console.log(
    `${result.upsertedCount || 0} new parking slots added. A-01 to A-20 are ready at Level A.`,
  );
  process.exit(0);
};
