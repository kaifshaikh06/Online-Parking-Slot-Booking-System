...
const { isBookingReserved, releaseExpiredBookings } = require('../services/bookingExpiryService');
...
const validateSlot = ({ slotNumber, location, vehicleType, price, status }) => {
  ...
  if (!carTypes.includes(vehicleType)) return 'Choose one of the supported car types.';
...
const updateSlot = async (req, res, next) => {
  try {
    ...
    if (reservationLocked) {
      const changedProtectedDetails = req.body.vehicleType !== slot.vehicleType
        || Number(req.body.price) !== slot.price
        || req.body.status !== slot.status;
      if (changedProtectedDetails) {
        return res.status(400).json({ message: 'This slot has a current or upcoming reservation. Only its slot number can be changed.' });
      }
      slot.slotNumber = requestedSlotNumber;
      await slot.save();
      return res.json({ message: 'Booked slot number updated successfully. Other slot details remain locked.', slot });
    }
