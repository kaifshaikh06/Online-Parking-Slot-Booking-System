...
const ManageSlots = () => {
  ...
  return <main className="container page"><div className="page-heading"><div><p className="eyebrow">Admin area</p><h1>Manage Parking Slots</h1></div><Link className="button" to="/admin/slots/new">Add Slot</Link></div><Message>{error}</Message><Message type="success">{success}</Message>{!slots ? <Loading /> : slots.length === 0 ? <p className="empty-state">No parking slots found. Run the slot seed to add A-01 to A-20.</p> : <div className="table-wrap"><table><thead><tr><th>Slot Number</th><th>Location</th><th>Vehicle Type</th><th>Hourly Rate</th><th>Status</th><th>Actions</th></tr></thead><tbody>{slots.map((slot) => <tr key={slot._id}><td>{slot.slotNumber}</td><td>{slot.location}</td><td>{slot.vehicleType}</td><td>₹{slot.price}/hr</td><td><StatusBadge status={slot.status} /></td><td className="actions"><Link className="button button-small button-outline" to={`/admin/slots/${slot._id}/edit`}>Edit</Link><button className="button button-small button-danger" onClick={() => remove(slot)}>Delete</button></td></tr>)}</tbody></table></div>}</main>;
};
