...
const EditSlot = () => {
  ...
  return <main className="container page"><div className="page-heading"><div><p className="eyebrow">Admin area</p><h1>Edit Parking Slot</h1></div></div><Message>{error}</Message>{slot ? <SlotForm initialValues={{ slotNumber: slot.slotNumber, location: 'Level A', vehicleType: slot.vehicleType, price: slot.price, status: slot.status }} reservationLocked={slot.reservationLocked} onSubmit={save} submitLabel={slot.reservationLocked ? 'Update Slot Number' : 'Save Changes'} /> : !error && <Loading />}</main>;
};
